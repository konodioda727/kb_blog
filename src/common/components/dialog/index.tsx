...
import {DialogInputProps, DialogTemplateProps} from "@/common/components/dialog/types";
...
export const DialogTemplate: React.FC<DialogTemplateProps> = (props) =>  {
    const {inputs, children, onClose, onConfirm, withClose, enterButtonText, desc, title} = props
        ...
        <>
            <Dialog open={isOpen} onOpenChange={setIsOpen}>
                <DialogTrigger asChild>
                    <Button variant="outline" onClick={() => setIsOpen(true)}>{enterButtonText && title}</Button>
                </DialogTrigger>
                <DialogContent className="sm:max-w-[425px]">
                    ...
                    <div className="grid gap-4 py-4">
                        {inputs && inputs.map((inputProps, index) =>  <DialogInput key={genKey.next().value as number} {...inputProps} />)}
                    </div>
                    {children}
                    {elem}
                    <DialogFooter>
                        <Button type="submit" onClick={handleConfirm}>确认</Button>
                        {withClose && <DialogClose asChild><Button onClick={handleClose} type="button">关闭</Button></DialogClose>}
                    </DialogFooter>
                </DialogContent>
            </Dialog>
        </>
